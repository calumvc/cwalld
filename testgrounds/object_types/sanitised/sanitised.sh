...
if [ $# -eq 1 ]; then
	if [ "$1" = "--update" ] ; then
		time=`ls -l --time-style="+%x %X" sanitised.te | awk '{ printf "%s %s", $6, $7 }'`
		rules=`ausearch --start $time -m avc --raw -se sanitised`
		if [ x"$rules" != "x" ] ; then
			...
			if [ "$ANS" = "y" -o "$ANS" = "Y" ] ; then
				...
				echo -e "$rules" | audit2allow -R >> sanitised.te
...
set -x
make -f /usr/share/selinux/devel/Makefile sanitised.pp || exit
/usr/sbin/semodule -i sanitised.pp
