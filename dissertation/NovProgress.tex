...
completion of the closest possible implementation is the main priority of the project. This means that this paper may be a proof of concept rather than full implementation.
...
Now information \textit{cannot} flow from Bank A to B. The user is still able to access either Oil Company object. This presents a flaw with the Chinese Wall model, if companies A and B are still in conflict,
information could still travel to Oil Company B. This flaw will however not be addressed in this paper, as it is focused solely on the implementation of the Wall.
...
Security Enhanced Linux implements a Mandatory Access Control (MAC) system extended of the classic
...
control anything. DAC in UNIX leverages the \verb_chmod_ (change mode) command, allowing read, write and execute access of objects to be edited for the 3 main groups subjects can belong to. DAC brings about great
...
MAC is the other model that shares the spotlight with DAC in system security. It uses labels such that only subjects of 
the same label can perform operations on the object, as defined by policies installed on the system.
...
being an open source alternative to the UNIX that was locked down by AT&T. Linus Torvalds combined his Linux kernel with the GNU system tools to create GNU/Linux, leading to what we now know as Linux.
...
This will be the longest phase, including daemon development, along with testing. Phase 2 will also likely venture into Kernel level engineering, as a Chinese Wall will require atomic operations to deny subjects
