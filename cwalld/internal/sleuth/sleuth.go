	...
	go func() { // run this part concurrently
		for line := range t.Lines { // auditd has 2 parts, the syscall and path, we are going to combine them into a struct
			...
			if strings.Contains(line.Text, "cwalld"){ // this is the syscall part, containing pid, operation and subject name
				...
				audit_id := regex.FindStringSubmatch(line.Text)

				regex = regexp.MustCompile(`\ba2=(\d+)`)
				operation := regex.FindStringSubmatch(line.Text)

				flags, err := strconv.ParseInt(operation[1], 16, 64) // convert the string that is hexadecimal, into straight binary, which is read as an int64 but actually is just straight flags 
				utils.CheckErr(err, true)
				...
				if subject == nil { // add it to the global list of subjects if not
					subject = &utils.Subject{ Pid: pid[1], Name: subject_name[1] }
					subject.ToString()
					*subjects = append(*subjects, *subject)
				}

				audit := &utils.Audit{ Id: audit_id[1], Subject: subject, Directory: "", Operation: op } // create new audit
			...
			if strings.Contains(line.Text, DIR){ // this is the path line, containing the affected object path
				...
				directory := regex.FindStringSubmatch(line.Text)

				for _, a := range *audits {
					if a.Id == audit_id[1] {
						a.Directory = directory[1]
						a.ToString()
						break
					}
				}
